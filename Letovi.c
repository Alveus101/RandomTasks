...
int main() {
	...
	if (presjedanjeID > -1)
		printf("Ukupno: %.2f\nPolazak u %s\nPresjedanje: %s", l[letID].cijena+l[presjedanjeID].cijena, formatTime(time, l[letID].polazakS, l[letID].polazakM), l[presjedanjeID].polaziste);
	
	else printf("Ukupno: %.2f\nPolazak u %s", l[letID].cijena, formatTime(time, l[letID].polazakS, l[letID].polazakM) );
